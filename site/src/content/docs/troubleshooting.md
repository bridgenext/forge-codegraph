...
- **You're on an old (pre-0.9) install.** Reinstall to get the bundled runtime — `curl -fsSL https://raw.githubusercontent.com/bridgenext/forge-codegraph/main/install.sh | sh` (macOS/Linux), `irm https://raw.githubusercontent.com/bridgenext/forge-codegraph/main/install.ps1 | iex` (Windows).
...
The MCP server auto-syncs on save (wait a couple of seconds). Run `codegraph sync` manually if needed. Check that the file's language is [supported](/forge-codegraph/reference/languages/) and isn't inside a `.gitignore`d or default-excluded directory (e.g. `node_modules`, `dist`).
